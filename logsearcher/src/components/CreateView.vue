...
export default {
    data() {
        const filterName = ""
        const search = ""
        const cols = [{ name: "Data", query: "logdata" }]
        const loading = false
...
<template>   
    ...
    <div class="flexdiv">
      ...
      <label for="filter-where">View WHERE clause</label><input type="text" name="filter-where" class="largesearch" v-model="search">  
    </div>
    <button @click="createView()">Create view</button><button @click="$emit('cancel-view')" class="secondary">Cancel</button>
  </div>
  <div v-else>
    LOADING
  </div>
